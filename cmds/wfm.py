...
class wfm(Cog_Extension):
  tag = "Warframe"
  @commands.command(name='translate',aliases=['trans','翻譯'],brief="進行中英文翻譯",description="翻譯`item`於[wf.m](https://warframe.market)上的名稱")
  async def translate(self,ctx,*item):
    ...
    await ctx.send(message)
  @commands.command(name='wfm', aliases=['wm', '市場查詢'],brief="查詢物品價格",description="查詢`item`於[wf.m](https://warframe.market)上的價格")
          ...
          if count > 0:
            ...
            if orders['order_type'] == order_type and user['status'] == 'ingame' and orders['platform'] == 'pc':
              ...
              if rank != "":
                ...
                ChiRank = ""
              embed = DiscordEmbed(title=f"物品:{itemName}\t數量:{orders['quantity']}\t{ChiRank}",description=f"價格:{int(orders['platinum'])}",color=0x3b859a)
