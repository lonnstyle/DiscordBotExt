          ...
          if count > 0:
            ...
            if orders['order_type'] == order_type and user['status'] == 'ingame' and orders['platform'] == 'pc':
              ...
              if rank != "":
                ...
                localRank = ""
              embed = DiscordEmbed(title=lang['wfm.embed.title'].format(itemName=items,quantity=orders['quantity'],localRank=localRank,tax=tax,ducats=ducats),description=lang['wfm.embed.description'].format(platinum=int(orders['platinum'])),color=0x3b859a)
              embed.add_embed_field(name=lang["wfm.embed.field.cpMsg"], value = f"\n/w {user['ingame_name']} Hi! I want to {action}: {itemName} {rank} for {int(orders['platinum'])} platinum. (warframe.market)\n")
              avatar = user['avatar']
              if avatar == None:
                avatar = "user/default-avatar.png"
              embed.set_author(name=user['ingame_name'], icon_url="https://warframe.market/static/assets/"+avatar, url = f"https://warframe.market/{lang['wfm.link.language']}/profile/"+user['ingame_name'])
          ...
          if count > 0:
            ...
            if orders['order_type'] == order_type and user['status'] == 'ingame' and orders['platform'] == 'pc':
              ...
              if rank != "":
                ...
                localRank = ""
              message+=lang['wfm.message.item'].format(user=user['ingame_name'],itemName=items,quantity=orders['quantity'],localRank=localRank,platinum=int(orders['platinum']),tax=tax,ducats=ducats)
